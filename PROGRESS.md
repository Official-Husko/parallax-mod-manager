...
The detailed, kept-current build log behind [Parallax Mod Manager](README.md)'s short
[Features](README.md#features) and [Roadmap](README.md#roadmap) sections - every feature that
landed, and everything known but not yet built, each with the reasoning and the real install it
was confirmed against, where that applies. Written for whoever wants the full story; the main
README stays the short version.
...
- **Settings > Conflict rules** - which content types are FIOS instead of the LIOS default.
  ...
  regression test pins it to exactly that); twelve are confirmed so far, all Stellaris:
  ...
  latest); `common/component_sets`, `common/component_templates`, `common/global_ship_designs`,
  `common/scripted_variables`, `events`, `common/ship_behaviors`, `common/special_projects`,
  `common/solar_system_initializers`, `common/start_screen_messages` and `common/event_chains`
  (each cross-confirmed against both the wiki and an existing reference implementation's own FIOS
  folder list); and `common/scripted_loc` (wiki-confirmed alone, held to the same single-source
  bar as static_modifiers). Rather than this project trying to divine the rest centrally, a
  person's own per-Type override (`internal/priorityrules`, one JSONC file per game) layers on top
  of the built-in defaults - `library.Options.RuleOverrides`, merged the same way for both a scan
  and a generated patch (`internal/library`'s own `effectiveRules`, the one place that combination
...
- **Playset sharing via codes** - the Library screen's collections and bulk actions are now real
  (see the Done list above); encoding/decoding a playset as a shareable local code
