...
func checksumApp(t *testing.T, algorithm string) (*App, game.GameConfig, string) {
	t.Helper()
	// Relative to this package's own directory (internal/app), not the repo root -
	// go test always runs with the package's own directory as its working
	// directory.
	tree, err := filepath.Abs(filepath.Join("..", "checksum", "testdata", "tree"))
	if err != nil {
		t.Fatal(err)
	}
	data := t.TempDir()
