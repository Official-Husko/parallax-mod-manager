...
type App struct {
	...
	// The build's embedded data, handed in by New - main.go is the only file left
...
// backgroundSource, gameMedia and patchThumbnail are this build's embedded data -
// main.go holds the actual //go:embed directives (see this package's own doc
// comment) and passes them straight through here unchanged. Real setup
// (resolving OS paths, loading the games list) happens in Startup, once a
// context exists - see that method.
func New(gamesList []byte, licence string, backgroundSource []byte, gameMedia embed.FS, patchThumbnail []byte) *App {
	return &App{
		embeddedGamesList:        gamesList,
		embeddedLicence:          licence,
		embeddedBackgroundSource: backgroundSource,
		embeddedGameMedia:        gameMedia,
		embeddedPatchThumbnail:   patchThumbnail,
	}
}

// startup is called when the app starts. The context is saved so we can
// call the runtime methods. Deliberately unexported (see the exported Startup
// function below) - a bound Wails value's own exported methods are exactly what
// Wails generates a frontend binding for, and this is a lifecycle hook, not part
// of that API surface.
func (a *App) startup(ctx context.Context) {
	...
	registry, notice, err := game.LoadRegistry(AppVersion, a.embeddedGamesList, gamesListPath)
	if err != nil {
	...
	mediaFS, err := fs.Sub(a.embeddedGameMedia, "data/game_media")
	if err == nil {
...
// Startup is main.go's own way into startup, from across the package boundary -
// a plain function, not a method, so Wails' Bind reflection (which only scans a
// bound value's own method set) never sees it and tries to generate a frontend
// binding for it, the same reason unexported methods on this package's App never
// showed up as bindable before this package existed.
func Startup(a *App, ctx context.Context) { a.startup(ctx) }

// shutdown is called when the app is closing, before the runtime exits. See
// startup's own comment for why this stays unexported, with Shutdown below as
// the plain function main.go actually calls.
...
// Shutdown is main.go's own way into shutdown - see Startup's own comment.
func Shutdown(a *App, ctx context.Context) { a.shutdown(ctx) }
...
func (a *App) AboutInfo() about.Info {
	...
	info.LicenceName, info.LicenceID = about.ParseLicence(a.embeddedLicence)
	return info
}
...
func (a *App) patchThumbnail() []byte {
	...
	return a.embeddedPatchThumbnail
}
