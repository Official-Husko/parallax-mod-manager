...
func (a *App) initBackgrounds(configAppDir string) {
	...
	src, notice, err := backgrounds.LoadSource(a.embeddedBackgroundSource, override)
	if err != nil {
		log.Errorf("%v", err)
	}
	if notice != "" {
...
// Deliberately unexported (see app.go's startup/Startup for why, and
// BackgroundMiddleware below as the plain function main.go actually calls).
...
// BackgroundMiddleware is main.go's own way into backgroundMiddleware.
func BackgroundMiddleware(a *App) func(http.Handler) http.Handler {
	return a.backgroundMiddleware
}
