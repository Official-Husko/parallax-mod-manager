...
func WriteState(order conflict.LoadOrder, mods []mod.Mod, cfg game.GameConfig, opts Options) (Result, error) {
	...
	enabledMods, err := resolveEnabledMods(order, mods)
	if err != nil {
		return Result{}, err
	}
...
// resolveEnabledMods deduplicates order (first occurrence wins, mirroring
// conflict.LoadOrder.Priority's own tie-break), validates every id against
// mods, and builds each entry's descriptor path as the game engine expects
// it - "mod/<id>.mod", always forward-slashed (this is data inside a JSON
// file the Clausewitz engine parses, not a native OS path: using
// filepath.Join here would silently emit backslashes on a Windows build
// and produce a file the game can't resolve).
func resolveEnabledMods(order conflict.LoadOrder, mods []mod.Mod) ([]string, error) {
	...
	for _, id := range order {
		...
		entries = append(entries, path.Join("mod", id+".mod"))
	}
