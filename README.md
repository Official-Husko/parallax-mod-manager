...
playset active. Still missing: writing resolved conflicts back out as a patch mod and support for
more than one game. Nothing here is ready to fully replace your existing mod manager yet.
...
- **Per-game configuration** (`internal/game`) - Stellaris seeded; executable discovery via
  the Paradox Launcher's own `launcher-settings.json`, with a fallback. The user-data
  ...
  native Linux binary, no Proton involved.
...
- **Playsets and game launching** (`internal/playset`, `internal/library`, `app.go`,
  ...
  of version control; that mockup also sketches a larger product (a cross-game library
  browser, DLC management, a file-level conflict resolver, playset sharing, an update checker,
  a first-run wizard) that is deliberately not built yet - only its core workspace screen and
  visual system were adopted so far. `app.go`'s `LaunchGame` is the one function in the whole
...
- **JSON-launcher-format games** (Victoria 3, EU5) - `content_load.json`/`playsets.json`'s
  schemas are still unconfirmed against a real install of a game that uses them.
- **More than one game** - only Stellaris is registered in `internal/game` so far.
- **Everything else in the design mockup beyond the core workspace** - a cross-game library
  browser, DLC management, a settings screen with an attributable autosort rule engine, a
  file-level conflict resolver with an overlap matrix, playset sharing via codes, an update
  checker, and a first-run wizard. See `mockup/Mod Manager.dc.html` (local reference file,
  git-ignored) for what these are expected to look like when they're built.
